...
const getAuthorDates = async (commits: Array<string | undefined>): Promise<Array<string | undefined>> => {
	const filteredCommits = commits.filter((commit): commit is string => Boolean(commit && /^[0-9a-f]+$/i.test(commit)));
...
async function init(): Promise<void> {
	let commitElements: NodeListOf<HTMLElement> | HTMLElement[] = [];
...
async function addAuthoredDates(commitElements: NodeListOf<HTMLElement> | HTMLElement[], commitHashes: Array<string | undefined>): Promise<void> {
...
async function updateCommits(containers: HTMLElement[]): Promise<void> {
	...
	await addAuthoredDates(containers, commitHashes);
}
